...
- [✨ 核心特性](#-核心特性)
- [⚠️ 常见问题与注意事项](#%EF%B8%8F-常见问题与注意事项)
- [📖 使用指南](#-使用指南)
  - [1. 运行方式](#1-运行方式)
...
*   **智能跨域拦截与追加重试 (CORS)**：未开启代理时若遭遇跨域拦截，工具将自动**收集被拦截的链接**；并在转换完成后生成专属重试按钮，一键通过代理无缝**追加转换结果**到源数据末尾，彻底解决漏配问题。
*   **类型与模型智能推断**：
...
> 💡 **新增跨域智能追加机制**：
> 如果您在**未开启**代理的情况下开始转换，并在过程中遇到了 CORS 拦截，程序不再将其直接计为失败。相反，工具会在常规转换完成后：
> 1. **自动打开**界面上的跨域代理开关。
> 2. 在原转换按钮旁生成一个**【跨域代理转换 (X)】**按钮。
> 3. 您只需此时输入代理地址，点击该按钮，程序会自动重试被拦截的请求，并将成功的数据**完美无缝追加到已生成的 JSON 末尾**。

**强烈建议使用自建的 Cloudflare Worker 作为代理**，以保证您的 API Key 绝对安全，不经过任何第三方公共服务器。
